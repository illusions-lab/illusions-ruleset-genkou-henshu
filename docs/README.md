...
3. `docs/rules/<ruleId>.md` を [docs/rules/geh-douin-taishoutaisho.md](./rules/geh-douin-taishoutaisho.md) のテンプレートに
   従って作成する（**ルールごとに1ファイル**）。
...
- `docs/rules/` のファイル名は `ruleId` と一致させる（例 `geh-douin-taishoutaisho` → `docs/rules/geh-douin-taishoutaisho.md`）。
